...
import com.itmy.constant.Constants;
import com.itmy.exception.ParamException;
import com.itmy.picture.entity.FileSort;
import com.itmy.picture.service.IFileSortService;
import com.itmy.picture.service.ILocalFileService;
import com.itmy.picture.utils.DateUtils;
import com.itmy.picture.utils.FileUtils;
...
public class LocalFileServiceImpl implements ILocalFileService {
    ...
    @Autowired
    private IFileSortService fileSortService;
    /**
     * 本地图片上传路径
     */
    ...
    public String uploadPictureByUrl(String itemUrl, FileSort fileSort) {
        ...
        try {
            ...
            throw new ParamException("获取图片超时，文件上传失败");
        } finally {
            try {
                // 完毕，关闭所有链接
                os.close();
    ...
    private String uploadSingleFile(MultipartFile multipartFile, FileSort fileSort) throws IOException {
        ...
        // 保存本地，创建目录
        File file1 = new java.io.File(newPath);
        if (!file1.exists()) {
            file1.mkdirs();
        }
